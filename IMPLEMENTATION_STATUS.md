...
Removed palette/visual-phase changes from the whole-scene reset condition. Only entering a different run resets the world. Ordinary palette targets now blend existing track/scenery materials with 0–8.8 s stagger and 10 s interpolation; active-time scheduling stops progression during pause. Mirrors/safe drops can accelerate the same retained-object transition to 1.4–1.8 s. Hazards/currency/avatar are excluded from palette recolouring to preserve semantic cues and cosmetics.

The existing sky texture blends its tint while the lighting resource remains bound, avoiding synchronous sky-resource generation at palette boundaries. Void density interpolates rather than instantly hiding the horizon. Distant landmarks no longer replace the entire skyline at a palette or 192 m boundary: retained landmarks are translated on rebasing, and at most one empty/retired slot is populated per frame after its predecessor is behind the runner. New near scenery arrives through the normal chunk stream. The three-hour state changes no longer rebuild the scene; the existing nonterminal density progression and white ending remain.
