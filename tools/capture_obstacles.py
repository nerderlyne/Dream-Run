...
shots=['stairs','brokenFloor','exposedBridge','window','moon','collapse','furniture','volley','lightning','lightning-strike']
if len(sys.argv)>2:shots=[sys.argv[2]]
for shot in shots:
 token=str(uuid.uuid4()).upper()
 extra=['--strike'] if shot.endswith('-strike') else []
 sim('launch','--terminate-running-process',device,'nani.Dream-Again','--art-review','--design-review','--obstacle',shot.removesuffix('-strike'),'--collage-capture-token',token,*extra)
