...
Each distinct sports-ball impact after the contact lockout removes one straw limb: both arms before either leg, with impact side choosing the available limb. The first three also stumble and briefly slow the runner; the fourth unrepaired loss causes a 2.4-second rising straw-burst wake-up. Damage does not heal without straw. A rolling hay ball always gives +1 hay, including at full health. It restores one missing limb immediately or banks the straw; banked straw rebuilds one limb after a one-second delay following damage. Hay is a non-purchasable repair resource, not currency. Hay balls roll toward the player at 8 m/s from a 72-metre preview range and use swept collision. These are reclaimed humanoid fragments outside the 42 scenery families, generated on safe hazard-free support. Reserve straw, damage, roll-start ticks and collected IDs persist in live suspension. A continue rebuilds the body. Edge knockback and other lethal hazards still wake immediately. Cosmetic limb loss never changes the collision capsule or jump/slide controls.
