...
Version-one mechanics load and validate against the bundled data contract. Changes require a new version implementation and migration/reproducibility evidence; the app deliberately refuses altered R1 movement defaults. All geometry, the procedural icon and synthesized tones are original code-generated assets. Six supplied mood images remain only in `references/` and are not bundled.
