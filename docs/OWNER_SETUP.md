...
Version-one mechanics load and validate against the bundled data contract. Changes require a new version implementation and migration/reproducibility evidence; the app preserves R1 movement defaults and supports R2 with a 12.25 m/s initial speed. All geometry, the procedural icon and synthesized tones are original code-generated assets. Six supplied mood images remain only in `references/` and are not bundled.
